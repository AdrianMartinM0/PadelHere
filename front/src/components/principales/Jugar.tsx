...
const Jugar = () => {
  ...
  const fetchPartidos = async () => {
    ...
    try {
      const res = await fetch("https://padelhere.onrender.com/v1/partido/")
  ...
  const handleCreateMatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://padelhere.onrender.com/v1/partido/?user_id=${userId}`,
        {
          method: "POST",
  ...
  const handleLeave = async (partidoId: string, slot: string) => {
    try {
      ...
      const res = await fetch(`https://padelhere.onrender.com/v1/partido/${partidoId}/leave/${slot}?user_id=${userId}`, {
        method: "POST"
      });
